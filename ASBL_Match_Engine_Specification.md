# ASBL 比賽引擎規格書 (v1.6) ASBL_Match_Engine_Specification.md
...
## 5. 比賽引擎：投籃命中與結算 (Shooting & Scoring)

### 5.1 命中率判定 (Hit Rate Calculation)
若前場階段未發生失誤或被封蓋，則進入命中判定。

#### A. 屬性池定義
*   **進攻方總和 (Off_Total)**:
    *   `力量` + `速度` + `彈跳` + `手感` + `出手速度` + `進攻智商` + `運氣` + `投籃準心` + `射程` + `跑位` + `運球` + `傳球` + `控球` (共 13 項)。
*   **防守方總和 (Def_Total)**:
    *   `力量` + `速度` + `彈跳` + `手感` + `出手速度` + `防守智商` + `運氣` + `籃板` + `卡位` + `干擾` + `抄截` + `跑位` (共 12 項)。

#### B. 計算公式
*   **命中率**: `(40% + (Off_Total - Def_Total) / Def_Total)*(1 + 空間加成*0.1)*(1 + 品質加成))`。 **(V1.2 已更新基礎命中率由50% -> 40%)**
...
*   觸發時機: 進入 Spec 5.1 命中率判定之前。
*   候選人: 進攻方場上 5 人。
*   權重計算 (Weight):
*   基礎權重: 引用 Spec 5.1.A 之 Off_Total 屬性池 (共13項屬性總和)。
*   特殊加成:
...
*   結算:
    ...
    *   無論命中與否: 獲得 FGA, (3PA)。
### 6.2 罰球 (FTA, FTM)
*   觸發時機: Spec 3.5 (快攻犯規) 或 Spec 5.2 (投籃犯規)。
*   歸屬邏輯:
